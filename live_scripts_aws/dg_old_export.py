...
tables = [
    ("tender", f"update_date BETWEEN '{yesterday_date}' AND '{today_date}' and script_name not like '%archive%' and notice_text is not null and ((date(notice_deadline) is not null or date(notice_deadline) >= date(now()) )  or notice_type  in (7)) and is_publish_on_gec is true and (completed_steps != 'deleted' or completed_steps is null)"),
